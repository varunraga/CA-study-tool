...
- **PDFs now open full-width by default**, automatically collapsing the sidebar for a broader reading view — the biggest use of screen space, freed up the moment you open a PDF and automatically restored the moment you navigate anywhere else. A small "☰" button next to the PDF's breadcrumb lets you peek at the sidebar without leaving the PDF (it stays open across page turns until you close it again); on mobile, where the sidebar is already hidden by default, this doesn't change anything — the existing hamburger menu still works exactly as before.
